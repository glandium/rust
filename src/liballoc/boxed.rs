...
use crate::alloc::{Alloc as RealAlloc, Global, Layout, handle_alloc_error};
use crate::vec::Vec;
...
// pub to make the compiler happy wrt Alloc being made public by the impls below.
pub trait Alloc = RealAlloc + Default;

#[cfg(not(stage0))]
use RealAlloc as Alloc;
...
impl<A: Alloc> From<Box<str, A>> for Box<[u8], A> {
    ...
    fn from(s: Box<str, A>) -> Self {
        unsafe {
            let a = ptr::read(&s.1);
            Box::from_raw_in(Box::into_raw(s) as *mut [u8], stage0_unphantom(a))
        }
    }
}
    ...
    pub fn downcast<T: Any>(self) -> Result<Box<T, A>, Box<dyn Any, A>> {
        if self.is::<T>() {
            unsafe {
                ...
                Ok(Box::from_raw_in(raw as *mut T, stage0_unphantom(a)))
            }
        } else {
            Err(self)
        }
...
impl<A: Alloc> Box<dyn Any + Send, A> {
    ...
    pub fn downcast<T: Any>(self) -> Result<Box<T, A>, Box<dyn Any + Send, A>> {
        <Box<dyn Any, A>>::downcast(self).map_err(|s| unsafe {
            ...
            Box::from_raw_in(Box::into_raw(s) as *mut (dyn Any + Send), stage0_unphantom(a))
        })
    }
}
...
impl<T: ?Sized, A: Alloc> Receiver for Box<T, A> {}
...
pub trait FnBox<Args, A: Alloc = Global>: FnOnce<Args> {
...
impl<T: ?Sized + Unsize<U>, U: ?Sized, A: Alloc> CoerceUnsized<Box<U, A>>
    for Box<T, A> {}
...
impl<G: ?Sized + Generator + Unpin, A: Alloc> Generator for Box<G, A> {
    type Yield = G::Yield;
...
#[unstable(feature = "generator_trait", issue = "43122")]
impl<G: ?Sized + Generator, A: Alloc> Generator for Pin<Box<G, A>> {
    type Yield = G::Yield;
    type Return = G::Return;
